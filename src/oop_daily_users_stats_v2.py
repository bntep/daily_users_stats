...
Arguments:  -y or --year : a year or a list of year (optional, default is all years to current year)
            -l or --labo : the name of the laboratory (optional, default is all laboratories)
Example of usage:
...
def parse_arguments():
    ...
    if argyears is None:
            years = c_institution.data[c_institution.data['institution_name'] == labo]['year'].unique()
            if years is None:
                print(f"The institution '{labo}' does not have any activities for the year {years}.")
                sys.exit(1)
            
            years_int = list(map(int, years))           
            c_institution.create_institution_folder()           
            for year in years_int:
                print(f"\nCreating graphs for year: {year} \n")
                c_institution.create_and_save_graph(years=year) 
        ...
        years = list(year_int)            
        c_institution.create_institution_folder()
        l = [year for year in years if year not in set(c_institution.data['year'].tolist())]
        if l:
...
class Institution:
    ...
    def __init__(self, name: Union[str,list ]= None):
        self.data: pd.DataFrame = DF_STATS_INFOS
        self.name:  Optional[str] = None
        self.listname:  Optional[list] = None
        self.institution_folder:  Optional[str] = None
        
        all_institutions = set(self.data['institution_name'].unique())

        if name is None:
            # If no name is provided, initialize with the list of all institutions
            self.listname = sorted(list(all_institutions))
        elif ";" in name:
            # If a pipe-separated string of names is provided, split it into a list
            names = [n.strip() for n in name.split(";")]
            for n in names:
                if n not in all_institutions:
                    raise ValueError(f"The institution '{n}' does not exist in the database. Please check the spelling and the capitalization and try again.")
            self.listname = names            
        elif isinstance(name, str):
            # If a single name (str) is provided
            if name not in all_institutions:
                raise ValueError(f"The institution '{name}' does not exist in the database. Please check the spelling and the capitalization and try again.")
            self.name = name  
            self.listname = [name]
            #self.name = 
            #self.institution_folder = self.name.strip().replace(",", "").replace(" ", "_")
            #print(f"Creating institution Folder: {self.institution_folder} \n")        
        else:
            raise TypeError("Argument 'name' must be a string, a list of strings separated by '|', or None.")
    ...
    def create_institution_folder(self):
        ...
        self.institution_folder = institution_name
        CHEMIN_LABO = Path(CHEMIN_RESULTAT, institution_name)
        print(f"Creating folder for institution: {CHEMIN_LABO} \n")
    ...
    def treat_argument_year(self, argyears: str, labo: str):
        ...
        if argyears is None:
                years = self.data[self.data['institution_name'] == labo]['year'].unique()
                if years is None:
                    print(f"The institution '{labo}' does not have any activities for the year {years}.")
                    sys.exit(1)
...
if __name__ == "__main__": 
    ...
    # Create an iterator for the institutions
    c_institution = Institution(name=args.labo)
    
    # Iterate directly over the Institution object
    for labo in c_institution:
        print(f"Processing institution: {labo}")
        one_institution = Institution(name=labo)
        one_institution.treat_argument_year(args.year, labo)
    print("All institutions have been processed.")
